...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['salvar_perfil'])){
    ...
    if($nome===''||$email===''){
        ...
        try{
            $verifica=$pdo->prepare("SELECT id FROM usuario WHERE email=? AND id!=?");
            $verifica->execute([$email,$id_usuario]);

            if($verifica->fetch()){
                $mensagem="Este e-mail já está sendo usado.";
                $tipo_mensagem="erro";
            }else{
                $update=$pdo->prepare("UPDATE usuario SET nome=?,email=?,telefone=?,endereco=? WHERE id=?");
                $update->execute([$nome,$email,$telefone,$endereco,$id_usuario]);
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['cancelar_pedido'])){
    ...
    if($id_pedido>0){
        try{
            ...
            ");
            $cancelar->execute([$id_pedido,$id_usuario]);
...
$stmt=$pdo->prepare("
    SELECT id,nome,email,telefone,endereco,cpf
...
if($pagina==="pedidos"){
    $stmtPedidos=$pdo->prepare("
        SELECT
            id_pedido,subtotal,valor_entrega,
            CASE
                WHEN total IS NULL OR total=0
                THEN subtotal+valor_entrega
                ELSE total
            END AS total,
            tipo_entrega,forma_pagamento,status,data_pedido
        FROM pedido
        WHERE id_usuario=?
    ...
    if(!empty($pedidos)){
        ...
        $stmtItens=$pdo->prepare("
            SELECT
                ip.id_pedido,ip.quantidade,ip.preco_unitario,p.nome
...
<div id="modalExcluir" class="modal-excluir">
    <div class="modal-conteudo">
        <h2>Atenção!</h2>
        <p>Tem certeza que deseja apagar seu perfil? Esta ação não pode ser desfeita.</p>
        <div class="botoes-modal">
            <button type="button" class="btn-modal btn-cancelar" onclick="fecharModal()">Cancelar</button>
            <a href="deletar.php" class="btn-modal btn-confirmar">Sim, Apagar</a>
        </div>
    </div>
</div>

<div id="modalAcao" class="modal-excluir">
    <div class="modal-conteudo">
        <h2 id="modalAcaoTitulo">Atenção!</h2>
        <p id="modalAcaoTexto">Tem certeza que deseja continuar?</p>
        <div class="botoes-modal">
            <button type="button" class="btn-modal btn-cancelar" onclick="fecharModalAcao()">Não</button>
            <button type="button" id="btnConfirmarAcao" class="btn-modal btn-confirmar" onclick="confirmarAcaoModal()">Sim</button>
        </div>
    </div>
</div>
...
<header class="menu-superior">
    ...
    <a href="../html/tela-inicial.html" class="logo-site">
        <img src="../imagens/logo.png" alt="CerradoBurguer">
    </a>

    <nav class="links-menu">
        <a href="../html/tela-inicial.html">Início</a>
        <a href="../html/tela-inicial.html#promocoes">Promoções do dia</a>
        <a href="../html/cardapio.php">Cardápio</a>
        <a href="../html/tela-inicial.html#contato">Contato</a>
        <a href="../html/tela-inicial.html#avaliacao">Avaliações</a>
    </nav>
...
<aside class="perfil-menu">
    ...
    <a href="perfil.php" class="<?=($pagina==='perfil'||$pagina==='editar')?'selecionado':''?>">
        <i class="fa-solid fa-user"></i>
        <span>Meu Perfil</span>
    </a>

    <a href="perfil.php?pagina=pedidos" class="<?=$pagina==='pedidos'?'selecionado':''?>">
        <i class="fa-solid fa-receipt"></i>
        <span>Pedidos</span>
    </a>

    <a href="logout.php" onclick="return confirmarSaida(event)">
        <i class="fa-solid fa-right-from-bracket"></i>
        <span>Sair da Conta</span>
    </a>

    <a href="#" class="apagar" onclick="abrirModal(event)">
        <i class="fa-solid fa-trash"></i>
        <span>Apagar Perfil</span>
    </a>

    <div class="propaganda">
        <span>Peça seu<br>favorito<br>agora!</span>
    </div>

    <a href="../html/tela-inicial.html" class="voltar-inicio">
        Voltar ao início
    </a>
...
<section class="perfil-caixa editar-caixa">
    ...
    <form method="POST">
        ...
        <div class="campo">
            <label>CPF:</label>
            <input type="text" value="<?=htmlspecialchars($usuario['cpf']??'')?>" readonly class="bloqueado">
        </div>
        ...
        <div class="botoes">
            <button type="submit" name="salvar_perfil" class="btn salvar">
                Salvar Alterações
            </button>
...
<section class="pedidos-caixa">
    ...
    <div class="filtros">
        ...
        <button onclick="filtrarPedidos('aberto')">Abertos</button>
        <button onclick="filtrarPedidos('finalizado')">Finalizados</button>
        <button onclick="filtrarPedidos('cancelado')">Cancelados</button>
    </div>
    ...
    <?php if(empty($pedidos)): ?>
        ...
        <div class="sem-pedidos">
            ...
            <a href="../html/cardapio.php" class="btn salvar">Ver Cardápio</a>
        </div>
    ...
    <?php else: ?>
        ...
        <?php foreach($pedidos as $pedido): ?>
            ...
            if(str_contains($status,'cancel')){
                $classeStatus=$grupoStatus="cancelado";
            }elseif(str_contains($status,'final')||str_contains($status,'entreg')){
                $classeStatus=$grupoStatus="finalizado";
            }else{
                $classeStatus=$grupoStatus="aberto";
            }

            $itens=$itensPedidos[$pedido['id_pedido']]??[];
            ?>

            <article class="pedido-card" data-status="<?=$grupoStatus?>">
                ...
                <div class="pedido-produtos">
                    ...
                    <?php else: ?>
                        ...
                        <p>Produtos do pedido não encontrados.</p>
                    ...
                    <div class="acoes-pedido">
                        ...
                        <form method="POST" class="form-pedido">
                            ...
                            <button
                                ...
                                onclick="abrirConfirmacaoPedido(this,<?=$pedido['id_pedido']?>)"
                            >
...
<section class="perfil-caixa">
    ...
    <div class="botoes">
        ...
        <a href="perfil.php?pagina=editar" class="btn salvar">
            ✎ Editar perfil
        </a>
...
let formularioAcao=null,linkSaida=null;

function abrirConfirmacaoPedido(botao,idPedido){
    formularioAcao=botao.closest("form");
    linkSaida=null;

    document.getElementById("modalAcaoTitulo").textContent="Cancelar pedido";
    document.getElementById("modalAcaoTexto").textContent=
        "Tem certeza que deseja cancelar o pedido #"+idPedido+"?";

    const confirmar=document.getElementById("btnConfirmarAcao");
    confirmar.textContent="Cancelar";
    confirmar.className="btn-modal btn-cancelar-pedido-modal";
...
function confirmarSaida(event){
    ...
    linkSaida=event.currentTarget;
    formularioAcao=null;

    document.getElementById("modalAcaoTitulo").textContent="Sair da conta";
    document.getElementById("modalAcaoTexto").textContent=
        "Tem certeza que deseja sair da sua conta?";

    const confirmar=document.getElementById("btnConfirmarAcao");
    confirmar.textContent="Sair";
    confirmar.className="btn-modal btn-confirmar";
...
function confirmarAcaoModal(){
    ...
    if(linkSaida){
        window.location.href=linkSaida.href;
        return;
    }
...
window.addEventListener("click",function(event){
    ...
    if(event.target===modalExcluir) fecharModal();
    if(event.target===modalAcao) fecharModalAcao();

});

function filtrarPedidos(filtro){

    document.querySelectorAll(".pedido-card").forEach(function(pedido){

        pedido.style.display=
            filtro==="todos"||pedido.dataset.status===filtro
                ?"block"
                :"none";
